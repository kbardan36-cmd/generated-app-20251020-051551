...
const customTools = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Search a specific social media platform for a query.',
      parameters: {
        type: 'object',
        properties: {
          ...
          platform: {
            type: 'string',
            description: 'The social media platform to search.',
            enum: ['twitter', 'reddit', 'linkedin', 'facebook']
          }
        },
...
export async function getToolDefinitions(env: any) {
...
async function performWebSearch(query: string, env: any, numResults = 5): Promise<string> {
  const apiKey = env.SERPAPI_KEY;
  if (!apiKey) {
    console.warn('SERPAPI_KEY is not set in the environment. Web search will be unavailable.');
    return 'The web search tool is not configured. Please contact the administrator to enable this feature by setting the `SERPAPI_KEY`.';
  }
  try {
...
async function fetchWebContent(url: string): Promise<string> {
  try {
    ...
    return `Failed to fetch content from ${url}. Reason: ${reason}`;
  }
}
    ...
    switch (name) {
      ...
      case 'social_media_search': {
        ...
        if (typeof query !== 'string' || typeof platform !== 'string') {
          return { error: 'Query and platform are required for social media search.' };
        }
        const siteQuery = `site:${platform}.com ${query}`;
        const content = await performWebSearch(siteQuery, env, 5);
        return { content };
      }
      default: {
