...
export const renderToolCall = (toolCall: ToolCall): string => {
  ...
  switch (toolCall.name) {
    ...
    case 'web_search':
      return `🔍 Searching web for "${toolCall.arguments.query}"...`;
    case 'social_media_search': {
      const platform = (toolCall.arguments.platform as string) || 'social media';
      const capitalizedPlatform = platform.charAt(0).toUpperCase() + platform.slice(1);
      return `🗣️ Searching ${capitalizedPlatform} for "${toolCall.arguments.query}"...`;
    }
    default:
